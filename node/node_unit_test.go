...
var _ = Describe("NodeUnitTest", func() {
	...
	Describe(".NewNode", func() {
		Context("address", func() {
			...
			It("return nil if address is ':40000'", func() {
				n, err := NewNode(cfg, ":40000", crypto.NewKeyFromIntSeed(1), log)
				Expect(err).To(BeNil())
				closeNode(n)
			})

			It("return nil if address is '127.0.0.1:40000'", func() {
				n, err := NewNode(cfg, "127.0.0.1:40000", crypto.NewKeyFromIntSeed(1), log)
				Expect(err).To(BeNil())
				closeNode(n)
			})
		})
	...
	Describe(".SetHardcodedState", func() {
		When("peer hardcoded status is false", func() {

			var n *Node

			BeforeEach(func() {
				n = NewAlmostEmptyNode()
				n.SetLocalNode(n)
