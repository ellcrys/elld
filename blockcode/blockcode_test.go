...
func TestBlockcode(t *testing.T) {
	...
	g.Describe("Blockcode", func() {

		g.Describe(".validateManifest", func() {
			g.It("should validate correctly", func() {
		...
		g.Describe(".FromDir", func() {

			g.It("should return error = 'project path does not exist' if project path is not found", func() {
				_, err := FromDir("./testdata/unknown")
				Expect(err).ToNot(BeNil())
				Expect(err.Error()).To(Equal("project path does not exist"))
			})

			g.It("should return error = 'failed to decode manifest: json: cannot unmarshal string into Go value of type blockcode.Manifest' if manifest is not valid JSON", func() {
			...
			g.It("should return error = ''package.json' file not found in {./testdata/missing_manifest}' if package.json is missing", func() {
			...
			g.It("should successfully create a Blockcode", func() {
		...
		g.Describe(".Hash", func() {
			g.It("should return Hash", func() {
				bc, err := FromDir("./testdata/blockcode_example")
				Expect(err).To(BeNil())
		...
		// 	g.It("should return err = 'destination path does not exist' if destination path does not exist", func() {
		// 		bc, err := FromDir("./testdata/blockcode_example")
		// 		Expect(err).To(BeNil())
		...
		// 	g.It("should successfully un-tar to destination", func() {
