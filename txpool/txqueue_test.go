...
var _ = Describe("Txqueue", func() {
	...
	Describe(".Append", func() {
		It("should return false when capacity is reached", func() {
			tx := wire.NewTransaction(wire.TxTypeRepoCreate, 1, "something", "pub_key", "0", "0", time.Now().Unix())
			q := NewQueue(0)
		...
		When("sorting is disabled", func() {
			It("should return transactions in the following order tx2, tx1", func() {
				tx1 := wire.NewTransaction(wire.TxTypeRepoCreate, 1, "something", "pub_key", "0", "0.10", time.Now().Unix())
				tx2 := wire.NewTransaction(wire.TxTypeRepoCreate, 1, "something", "pub_key", "0", "1", time.Now().Unix())
				q := NewQueueNoSort(2)
				q.Append(tx1)
				q.Append(tx2)
	...
	Describe(".Prepend", func() {
		...
		It("should return nil and add item at the head", func() {
			...
			tx2 := wire.NewTransaction(wire.TxTypeRepoCreate, 2, "something", "pub_key", "0", "0", time.Now().Unix())
		...
		When("sorting is disabled", func() {
			It("should return transactions in the following order tx2, tx1", func() {
				tx1 := wire.NewTransaction(wire.TxTypeRepoCreate, 1, "something", "pub_key", "0", "1", time.Now().Unix())
				tx2 := wire.NewTransaction(wire.TxTypeRepoCreate, 1, "something", "pub_key", "0", "0.10", time.Now().Unix())
				q := NewQueueNoSort(2)
				q.Prepend(tx1)
				q.Prepend(tx2)
				Expect(q.Size()).To(Equal(int64(2)))
	...
	Describe(".First", func() {
		...
		Context("with varying transaction fee", func() {
			It("should return first transaction in the queue and reduce queue size to 1", func() {
				q := NewQueue(2)
				tx := wire.NewTransaction(wire.TxTypeRepoCreate, 1, "something", "pub_key", "0", "0.2", time.Now().Unix())
				tx2 := wire.NewTransaction(wire.TxTypeRepoCreate, 2, "something", "pub_key", "0", "1", time.Now().Unix())
				q.Append(tx)
				q.Append(tx2)
	...
	Describe(".Sort", func() {
		It("should sort in ascending order", func() {
			q := NewQueue(3)
			q.Append(wire.NewTransaction(wire.TxTypeRepoCreate, 1, "something", "pub_key", "0", "0.2", time.Now().Unix()))
			q.Append(wire.NewTransaction(wire.TxTypeRepoCreate, 1, "something", "pub_key", "0", "0.5", time.Now().Unix()))
			q.Append(wire.NewTransaction(wire.TxTypeRepoCreate, 1, "something", "pub_key", "0", "1", time.Now().Unix()))
