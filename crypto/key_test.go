...
func TestKey(t *testing.T) {
	...
	g.Describe("Key", func() {
		...
		g.Describe(".NewKey", func() {
			g.When("seeds are '1'", func() {
				g.It("multiple calls should return same private keys", func() {
					seed := int64(1)
					a1, err := NewKey(&seed)
					Expect(err).To(BeNil())
					a2, err := NewKey(&seed)
			...
			g.When("seeds are nil", func() {
				g.It("should return random key on each call", func() {
					a1, err := NewKey(nil)
					Expect(err).To(BeNil())
					a2, err := NewKey(nil)
					Expect(err).To(BeNil())
			...
			g.When("seeds are different", func() {
				g.It("multiple calls should not return same private keys", func() {
					seed := int64(1)
					a1, err := NewKey(&seed)
					Expect(err).To(BeNil())
					seed = int64(2)
		...
		g.Describe(".PeerID", func() {
			g.It("should return 12D3KooWHHzSeKaY8xuZVzkLbKFfvNgPPeKhFBGrMbNzbm5akpqu", func() {
				seed := int64(1)
				a1, err := NewKey(&seed)
				Expect(err).To(BeNil())
		...
		g.Describe("PubKey.Bytes", func() {
			g.It("should return err.Error('public key is nil')", func() {
				a := PubKey{}
				_, err := a.Bytes()
			...
			g.It("should return []byte{111, 21, 129, 112, 155, 183, 177, 239, 3, 13, 33, 13, 177, 142, 59, 11, 161, 199, 118, 251, 166, 93, 140, 218, 173, 5, 65, 81, 66, 209, 137, 248}", func() {
				seed := int64(1)
				a, err := NewKey(&seed)
		...
		g.Describe("PubKey.Base58", func() {
			g.It("should return 48d9u6L7tWpSVYmTE4zBDChMUasjP5pvoXE7kPw5HbJnXRnZBNC", func() {
				seed := int64(1)
				a, err := NewKey(&seed)
				Expect(err).To(BeNil())
				hx := a.PubKey().Base58()
		...
		g.Describe("Priv.Bytes", func() {
			g.It("should return err.Error('private key is nil')", func() {
				a := PrivKey{}
				_, err := a.Bytes()
				Expect(err).NotTo(BeNil())
			...
			g.It("should return []byte{82, 253, 252, 7, 33, 130, 101, 79, 22, 63, 95, 15, 154, 98, 29, 114, 149, 102, 199, 77, 16, 3, 124, 77, 123, 187, 4, 7, 209, 226, 198, 73, 111, 21, 129, 112, 155, 183, 177, 239, 3, 13, 33, 13, 177, 142, 59, 11, 161, 199, 118, 251, 166, 93, 140, 218, 173, 5, 65, 81, 66, 209, 137, 248}", func() {
				seed := int64(1)
				a, err := NewKey(&seed)
		...
		g.Describe("Priv.Base58", func() {
			g.It("should return wU7ckbRBWevtkoT9QoET1adGCsABPRtyDx5T9EHZ4paP78EQ1w5sFM2sZg87fm1N2Np586c98GkYwywvtgy9d2gEpWbsbU", func() {
				seed := int64(1)
				a, err := NewKey(&seed)
				Expect(err).To(BeNil())
				hx := a.PrivKey().Base58()
		...
		g.Describe("Priv.Sign", func() {
			g.It("should sign the word 'hello'", func() {
				seed := int64(1)
				a, err := NewKey(&seed)
				Expect(err).To(BeNil())
		...
		g.Describe("Priv.Marshal", func() {
			g.It("should marshal and unmarshal correctly", func() {
				seed := int64(1)
				a, err := NewKey(&seed)
				Expect(err).To(BeNil())
		...
		g.Describe("Pub.Verify", func() {

			g.It("should return false when signature is incorrect", func() {
				seed := int64(1)
				a, err := NewKey(&seed)
				Expect(err).To(BeNil())
				sig, err := a.PrivKey().Sign([]byte("hello"))
			...
			g.It("should return true when the signature is correct", func() {
				seed := int64(1)
				a, err := NewKey(&seed)
				Expect(err).To(BeNil())
				sig, err := a.PrivKey().Sign([]byte("hello"))
		...
		g.Describe(".IsValidAddr", func() {
			g.It("should return error.Error(empty address)", func() {
				err := IsValidAddr("")
				Expect(err).ToNot(BeNil())
				Expect(err.Error()).To(Equal("empty address"))
			})

			g.It("should return err.Error(checksum error)", func() {
				err := IsValidAddr("hh23887dhhw88su")
				Expect(err).ToNot(BeNil())
				Expect(err.Error()).To(Equal("checksum error"))
			})

			g.It("should return err.Error(invalid version)", func() {
		...
		g.Describe(".IsValidPubKey", func() {
			g.It("should return error.Error(empty pub key)", func() {
				err := IsValidPubKey("")
				Expect(err).ToNot(BeNil())
		...
		g.Describe(".FromBase58PubKey", func() {
			g.It("should return err.Error(checksum error)", func() {
			...
			g.It("should return err = nil", func() {
		...
		g.Describe(".IsValidPrivKey", func() {
			g.It("should return error.Error(empty priv key)", func() {
				err := IsValidPrivKey("")
				Expect(err).ToNot(BeNil())
		...
		g.Describe(".PrivKeyFromBase58", func() {
			g.It("should return err.Error(checksum error)", func() {
