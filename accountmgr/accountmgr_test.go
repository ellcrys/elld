...
func TestAccountMgr(t *testing.T) {
	...
	g.Describe("AccountMgr", func() {
		...
		g.Describe(".hardenPassword", func() {
			g.It("should return [215, 59, 34, 12, 157, 105, 253, 31, 243, 128, 41, 222, 216, 93, 165, 77, 67, 179, 85, 192, 127, 47, 171, 121, 32, 117, 125, 119, 109, 243, 32, 95]", func() {
		...
		g.Describe(".askForPassword", func() {
			am := New(accountPath)

			g.It("should return err = 'Passphrases did not match' when passphrase and repeat passphrase don't match", func() {
				count := 0
			...
			g.It("should return input even when no password is provided the first time", func() {
				count := 0
		...
		g.Describe(".askForPasswordOnce", func() {
			am := New(accountPath)

			g.It("should return the first input received", func() {
				count := 0
		...
		g.Describe(".createAccount", func() {
			...
			g.It("should return err = 'Address is required' when address is nil", func() {
				err := am.CreateAccount(nil, "")
				Expect(err).ToNot(BeNil())
				Expect(err.Error()).To(Equal("Address is required"))
			})

			g.It("should return err = 'Passphrase is required' when passphrase is empty", func() {
				seed := int64(1)
			...
			g.It("should return nil when account has been created", func() {
				seed := int64(1)
				address, _ := crypto.NewKey(&seed)
				passphrase := "edge123"
			...
			g.When("account has been created", func() {
				...
				g.It("should have a keyfile in the account directory", func() {
				...
				g.It("should return err = 'Account already exist' when account with same address already exist", func() {
					seed := int64(1)
					address, _ = crypto.NewKey(&seed)
					passphrase := "edge123"
					err := am.CreateAccount(address, passphrase)
