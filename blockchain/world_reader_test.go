...
func TestWorldReader(t *testing.T) {
	g := Goblin(t)
	RegisterFailHandler(func(m string, _ ...int) { g.Fail(m) })

	g.Describe("WorldReader", func() {
		...
		g.Describe(".GetAccount", func() {
			...
			g.Describe("with no chain provided", func() {

				g.It("should return error if best chain is not set", func() {
					bc.bestChain = nil
				...
				g.It("should successfully get account", func() {
					account, err := wr.GetAccount(nil, util.String(sender.Addr()))
					Expect(err).To(BeNil())
					Expect(account).ToNot(BeNil())
				})

				g.It("should return err if account is not found", func() {
			...
			g.Describe("with a chain", func() {

				g.Context("chain has no parent", func() {
					g.It("should successfully get account", func() {
				...
				g.Context("chain with parents", func() {
					...
					g.Describe("account is created in block 2 and updated in block 3 of parent chain", func() {
						...
						g.It("should successfully get only account created on or before the parent's block number", func() {
							result, err := wr.GetAccount(sidechain1, util.String(account.GetAddress()))
							Expect(err).To(BeNil())
							Expect(result).To(Equal(account))
						})
					...
					g.Describe("account is created in block 3", func() {
						...
						g.It("should return ErrAccountNotFound when account does not exist on blocks created on or before the parent block number", func() {
							result, err := wr.GetAccount(sidechain1, util.String(account.GetAddress()))
							Expect(err).ToNot(BeNil())
							Expect(err).To(Equal(core.ErrAccountNotFound))
					...
					g.Describe("account is created in block 2 and updated in block 3 of grand parent chain", func() {
						...
						g.It("should successfully get only account created on or before the chain's parent's parent block number", func() {
							result, err := wr.GetAccount(sidechain2, util.String(account.GetAddress()))
							Expect(err).To(BeNil())
							Expect(result).To(Equal(account))
						})
