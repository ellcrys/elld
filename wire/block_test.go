...
var _ = Describe("Block & Header", func() {
	...
	Describe("Block.Validate", func() {

		var header1 = &Header{
			CreatorPubKey:    key.PubKey().Base58(),
			ParentHash:       RandString(66),
			Number:           2,
			TransactionsRoot: RandString(66),
			StateRoot:        RandString(66),
		...
		It("should return err when header fails validation", func() {
			...
			err := b.Validate()
			Expect(err).ToNot(BeNil())
			Expect(err.Error()).To(Equal("header error: field:parentHash, error:expected 66 characters"))
		})
		...
		It("should return err when transaction fails validation", func() {
			...
			err := b.Validate()
			Expect(err).ToNot(BeNil())
			Expect(err.Error()).To(Equal("transaction{0} error: field:hash, error:hash is required"))
		})

		It("should return err when a transaction signature is invalid", func() {
		...
		It("should return err when block hash is not valid", func() {
			...
			err = b.Validate()
			Expect(err).ToNot(BeNil())
			Expect(err.Error()).To(Equal("block error: hash not valid"))
		})
		...
		It("should return err when block signature is not set", func() {
			...
			err = b.Validate()
			Expect(err).ToNot(BeNil())
			Expect(err.Error()).To(Equal("block error: signature is not set"))
		})
	})
})
