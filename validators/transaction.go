...
func (v *TxsValidator) checkFormatAndValue(tx *wire.Transaction) (errs []error) {
	...
	if !funk.ContainsInt64(KnownTransactionTypes, tx.Type) {
		errs = append(errs, fieldErrorWithIndex(v.currentTxIndexInLoop, "type", "unsupported transaction type"))
	}

	// Nonce must be a non-negative integer
	if tx.Nonce < 0 {
		errs = append(errs, fieldErrorWithIndex(v.currentTxIndexInLoop, "nonce", "nonce must be non-negative"))
	}

	// Recipient's address must be set and it must be valid
	if tx.To == "" {
		errs = append(errs, fieldErrorWithIndex(v.currentTxIndexInLoop, "to", "recipient address is required"))
	} else if err := crypto.IsValidAddr(tx.To); err != nil {
		errs = append(errs, fieldErrorWithIndex(v.currentTxIndexInLoop, "to", "recipient address is not valid"))
	}

	// Sender's address must be set and it must be valid
	if tx.From == "" {
		errs = append(errs, fieldErrorWithIndex(v.currentTxIndexInLoop, "from", "sender address is required"))
	} else if err := crypto.IsValidAddr(tx.From); err != nil {
		errs = append(errs, fieldErrorWithIndex(v.currentTxIndexInLoop, "from", "sender address is not valid"))
	}

	// Sender public key is required and must be valid.
	if tx.SenderPubKey == "" {
		errs = append(errs, fieldErrorWithIndex(v.currentTxIndexInLoop, "senderPubKey", "sender public key is required"))
	} else if _, err := crypto.PubKeyFromBase58(tx.SenderPubKey); err != nil {
		errs = append(errs, fieldErrorWithIndex(v.currentTxIndexInLoop, "senderPubKey", "sender public key is not valid"))
	}
	...
	if tx.Type == wire.TxTypeBalance {
		if tx.Value == "" {
			errs = append(errs, fieldErrorWithIndex(v.currentTxIndexInLoop, "value", "value is required"))
		}
		_value, err := decimal.NewFromString(tx.Value)
		if err != nil {
			errs = append(errs, fieldErrorWithIndex(v.currentTxIndexInLoop, "value", "could not convert to decimal"))
		}
		if _value.LessThanOrEqual(decimal.Zero) {
			errs = append(errs, fieldErrorWithIndex(v.currentTxIndexInLoop, "value", "value must be greater than zero"))
		}
	}

	// Timestamp is required and cannot be a time in the future
	if tx.Timestamp == 0 {
		errs = append(errs, fieldErrorWithIndex(v.currentTxIndexInLoop, "timestamp", "timestamp is required"))
	} else if tx.Timestamp > time.Now().Unix() {
		errs = append(errs, fieldErrorWithIndex(v.currentTxIndexInLoop, "timestamp", "timestamp cannot be a future time"))
	}

	// Fee cannot be empty or less than 0
	if tx.Fee == "" {
		errs = append(errs, fieldErrorWithIndex(v.currentTxIndexInLoop, "fee", "fee is required"))
	} else {
		fee, err := decimal.NewFromString(tx.Fee)
		if err != nil {
			errs = append(errs, fieldErrorWithIndex(v.currentTxIndexInLoop, "fee", "could not convert to decimal"))
		} else if tx.Type == wire.TxTypeBalance && fee.LessThanOrEqual(constants.BalanceTxMinimumFee) {
			errs = append(errs, fieldErrorWithIndex(v.currentTxIndexInLoop, "fee", fmt.Sprintf("fee cannot be below the minimum balance transaction fee {%s}", constants.BalanceTxMinimumFee.StringFixed(8))))
		}
	}
	...
	if tx.Hash == "" {
		errs = append(errs, fieldErrorWithIndex(v.currentTxIndexInLoop, "hash", "hash is required"))
	} else if tx.Hash != tx.ComputeHash2() {
		errs = append(errs, fieldErrorWithIndex(v.currentTxIndexInLoop, "hash", "hash is not correct"))
	}

	// Check that signature is provided
	if tx.Sig == "" {
		errs = append(errs, fieldErrorWithIndex(v.currentTxIndexInLoop, "sig", "signature is required"))
	}
...
func (v *TxsValidator) checkSignature(tx *wire.Transaction) (errs []error) {
	...
	if err != nil {
		errs = append(errs, fieldErrorWithIndex(v.currentTxIndexInLoop, "sig", err.Error()))
	} else if !valid {
		errs = append(errs, fieldErrorWithIndex(v.currentTxIndexInLoop, "sig", "signature is not valid"))
	}

	return
}

// duplicationCheck checks whether the transaction exists in some
...
func (v *TxsValidator) ValidateTx(tx *wire.Transaction) []error {
	...
	if v.allowDuplicateCheck {
		errs = append(errs, v.duplicationCheck(tx)...)
	}
	return errs
}
